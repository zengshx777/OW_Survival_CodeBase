...
for (i in (1:n_simu)){
  ...
  ### Cox Q model
  if(cox.q.method){
    res.cox.q = cox.q.model.fit(Y,DELTA,X,Z,alpha = alpha.arg,truncate =  truncate, boot.time = 250)
  ...
  ## MSM model
  if(cox.msm.method){
    res.cox.msm = cox.msm.model.fit(Y,DELTA,X,Z,alpha = alpha.arg,truncate =  truncate, boot.time = 250)
