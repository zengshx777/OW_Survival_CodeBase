...
PSW.pseudo <- function(Y, Z, DELTA, X,
                       alpha=c(-1,1),
                       weight.type="IPW",
                       estimand.type="ASCE",
                       evaluate.time = NA,
  ...
  gps.fit = multinom(Z ~ -1+X, maxit = 500, Hess = TRUE, trace = FALSE)
  ...
  if (!is.na(ps.threshold))
  {
    ...
    keep.ind = which(apply(e,1,FUN=function(x){(min(x)>ps.threshold)&&(max(x)<(1-ps.threshold))}))
    keep.ratio = length(keep.ind)/N
    if (keep.ratio < 0.8)
    {
      warning(paste((1-keep.ratio)*100,"%(>=20%) observations are disgarded due to PS truncation",sep=""))
    }
    Y = Y[keep.ind]
    e = e[keep.ind,]
    Z = Z[keep.ind]
  ...
  A = matrix(unlist(lapply(Z,FUN=function(x){as.numeric(x==0:(J-1))})),byrow=T,ncol=J)  # A_ij matrix
  ...
  wj = function(theta){
    ...
    Eta = X %*% Theta
    if (weight.type == "OW"){
      return(1 / (1 + as.numeric(rowSums(exp(-Eta)))))
  ...
  for(j in 2:J){
    wj = function(theta){
      ...
      Eta = X %*% Theta
      if(weight.type == "OW"){
        return(as.numeric(exp(-Eta[,j-1])) / (1 + as.numeric(rowSums(exp(-Eta)))))
      }else if(weight.type == "IPW"){
        return((1+as.numeric(rowSums(exp(Eta))))/exp(Eta[,j-1]))
      }else if(weight.type == "ATT"){
        if(target.j >= 1){
    ...
    wdotj = jacobian(wj,thetah)
    Hmat = rbind(Hmat, c(colMeans(A[,j] * (pseudo.obs -muhat.h[j]) * wdotj)))
  }
  ...
  if(var.method == 1){
    ...
    if(estimand.type == "SPCE")
    {
      obs.approxi = obs.approxi+unlist(lapply(1:N,
                                              FUN=function(j){
                                                mean(unlist(lapply(1:N,FUN=function(i){
