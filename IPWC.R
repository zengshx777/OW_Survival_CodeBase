IPWC <- function(y.all, z.all, W.all, q.all,binary=0){
  ...
  return(list(TAU=TAU, SE=SE, ASD=ASD, PTRIM = PTRIM))
    ...
    # estimate ps
    fit <- glm(z ~ -1 + W, family = binomial(link = "probit"))
    ...
    V_un=mean(z*(y-mu1.h)^2/(e.h^2))/w1^2*K_odds[1]^2+
      mean((1-z)*(y-mu0.h)^2/((1-e.h)^2))/w0^2*K_odds[2]^2
    v=crossprod(((1-e.h)/e.h)*(y-mu1.h)*z,W)/(n*w1)*K_odds[1]+
      crossprod((e.h/(1-e.h))*(y-mu0.h)*(1-z),W)/(n*w0)*K_odds[2]
    v_odds=(V_un-v%*%(2*M1-M2)%*%t(v))/n
    
    V_un=mean(z*(y-mu1.h)^2/(e.h^2))/w1^2*K_diff[1]^2+
      mean((1-z)*(y-mu0.h)^2/((1-e.h)^2))/w0^2*K_diff[2]^2
    v=crossprod(((1-e.h)/e.h)*(y-mu1.h)*z,W)/(n*w1)*K_diff[1]+
      crossprod((e.h/(1-e.h))*(y-mu0.h)*(1-z),W)/(n*w0)*K_diff[2]
    v_diff=(V_un-v%*%(2*M1-M2)%*%t(v))/n

    
    V_un=mean(z*(y-mu1.h)^2/(e.h^2))/w1^2*K_risk[1]^2+
      mean((1-z)*(y-mu0.h)^2/((1-e.h)^2))/w0^2*K_risk[2]^2
    v=crossprod(((1-e.h)/e.h)*(y-mu1.h)*z,W)/(n*w1)*K_risk[1]+
      crossprod((e.h/(1-e.h))*(y-mu0.h)*(1-z),W)/(n*w0)*K_risk[2]
