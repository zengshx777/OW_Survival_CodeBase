...
AIPW.pseudo <- function(Y,
  ...
  formula.text = paste("pseudo.obs~",paste(paste(paste("X", 1:(ncol(
    X
  ) - 1), sep = "."), collapse = "+"),
  "Z",
  paste(paste(paste(
  ...
  sep = "+"))
  outcome.model  = glm(as.formula(formula.text), data = data.temp, family = gaussian(link = LINK.TEXT))
  mu.fit = matrix(NA, nrow = length(Z), ncol = J)
  residual.matrix = matrix(NA, nrow = length(Z), ncol = J)
  for (j in 0:(J - 1)) {
    data.use = data.temp
    data.use$Z = rep(factor(j, levels=0:(J-1)))
    mu.fit[, (j + 1)] = predict(outcome.model, newdata = data.use, type="response")
...
#     residual.matrix[, (j + 1)] = pseudo.obs - mu.fit[, (j + 1)]
#   }
#   mu.weight.ow = as.numeric(colSums(A * residual.matrix * w.ow) / colSums(A * w.ow))
#   mu.model.ow = as.numeric(colSums(mu.fit * tilt.h.ow) / sum(tilt.h.ow))
#   muhat.h.ow = mu.model.ow + mu.weight.ow
#   mu.weight.ipw = as.numeric(colSums(A * residual.matrix * w.ipw) / colSums(A * w.ipw))
#   mu.model.ipw = as.numeric(colSums(mu.fit * tilt.h.ipw) / sum(tilt.h.ipw))
#   muhat.h.ipw = mu.model.ipw + mu.weight.ipw
#   ind.alpha = combn(J, 2)
#   alpha.matrix = matrix(0, nrow = ncol(ind.alpha), ncol = J)
#   alpha.matrix[cbind(1:nrow(alpha.matrix), ind.alpha[1,])] = -1
#   alpha.matrix[cbind(1:nrow(alpha.matrix), ind.alpha[2,])] = 1
