###################################################################################################################################
##
## <January 28, 2015> @ Huzhang Mao
## Platform: Window 7, R version 3.3.3 (2017-03-06)
## 
...
GIPW.std.omega <- function(dat, form, weight, delta=0.002, K=4) {
  # Obtain std estimator from sandwich method
  #
  # Args
  #   dat: data on which analysis is to be performed
  #   form: formula for propensity score model
...
calc.omega <- function(ps, weight, delta=0.002, K=4, Z=NULL) {
  ...
  if (weight == "IPW") {
    ...
    ans <- calc.omega.MW(ps, delta=delta)
  } else if (weight == "ATT") {
    ans <- ps
  } else if (weight == "ATC") {
    ans <- 1-ps
...
calc.weight <- function( dat, ps.form, weight, delta=0.002, K=4 ) {
  # calculate PS for a data set
  #
  # Args
  #   dat: data
  #   ps.form: PS formula
  ...
  # Return
  #   A vector of PS weight
  if(weight=="UNWEIGHT")
  {
    return (rep(1,nrow(dat)))
  }
  out.ps <- ps.model(dat, as.formula(ps.form));
  ps.hat <- out.ps$ps.hat;  # estimated ps
  Q <- dat$Z * ps.hat + (1 - dat$Z) * (1 - ps.hat);  # denominator of generic weight
  omega <- sapply(ps.hat, calc.omega, weight = weight, delta=delta, K=K);
...
calc.ps.weight <- function( ps, Zi, weight ) {
  # calculate weight for individual PS
  #
  # Args
  #   ps: propensity score
  ...
  #   weight: PS weight name
  # Return
  #   A scalar of PS weight
  if (weight == "UNWEIGHT")
  {
    return (1)
  }
  omega <- calc.omega(ps=ps, weight=weight );
  ans <- omega/( Zi*ps + (1-Zi)*(1-ps) );
...
calc.omega.MW <- function (ps, delta=0.002) {
...
calc.W.derive.beta <- function(Zi, Xi, omega.ei, beta.hat, ei, Qi, weight, delta=0.002, K=4 ) {
  ...
  #
  # Returns
  #   A vetor, first order derivative of weight w.r.t. beta
