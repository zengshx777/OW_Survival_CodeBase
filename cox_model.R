...
cox.q.model.est <- function(Y,DELTA,X,Z,alpha=c(-1,1),truncate){
  ...
  cox.formula = as.formula(paste("Surv(Y,DELTA)~X",paste(paste("A",1:(J-1),sep="."),collapse="+"),sep="+"))
...
cox.q.model.fit <- function(Y,DELTA,X,Z,alpha=c(-1,1),truncate, boot.time = 250){
...
cox.msm.model.est<-function(Y,DELTA,X,Z,alpha = alpha,truncate){
  ...
  gps.model = multinom(Z ~ X, maxit = 500, Hess = TRUE, trace = FALSE)
  ...
  cox.formula = as.formula(paste("Surv(Y,DELTA)~",paste(paste("A",1:(J-1),sep="."),collapse="+"),sep=""))
...
cox.msm.model.fit <- function(Y,DELTA,X,Z,alpha=c(-1,1),truncate, boot.time = 250){
